...
#include<SerialisableObject.h>  // so we can put these in a BoostStore
...
class MTreeCut : public SerialisableObject {
	...
	template<class Archive> void serialize(Archive & ar, const unsigned int version){
		if(serialise){
			...
			ar & cut_description;
//			ar & ttree_entries;
//			ar & additional_indices;
			ar & current_entry;
			ar & tlist_entry;
			ar & type;
			// for type 1
			ar & additional_branchname;
			ar & linked_branch_list;
			ar & indexes_this_entry;
			// for type 2
			ar & additional_branchnames;
			ar & linked_branch_lists;
			ar & indices_this_entry;
		}
	}
	
	bool Print(){ return true; }; // dummy, required for SerialisableObjects
